...
public class Animacion {
    ...
    public void update (float deltaTime){   //tiempo de actualizacion
        tiempoFotogramas += deltaTime;
        if(tiempoFotogramas > maxTiempoFotogramas){
            fotograma++;    //Siguiente fotograma
            tiempoFotogramas = 0;
        }
        if(fotograma >= contFotogramas){ //Si llega al ultimo fotograma que reinicie y se vaya al primero
            fotograma = 0;
        }
    }

    public TextureRegion getFotograma(){
        return fotogramas.get(fotograma);   //Devolver el fotograma que necesitemos
    }
